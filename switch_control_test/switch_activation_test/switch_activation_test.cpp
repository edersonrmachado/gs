...
#define SWITCH_RX_PIN 24 // pin 17 
...
int main() {
    ...
    // Send message at each 2s 
    while(true) {
    
    hal->digitalWrite(SWITCH_RX_PIN, 1);// step 1 activate RF2 
    sleep(2)
    ...
    hal->digitalWrite(SWITCH_RX_PIN, 0); // step 3 activate RF1
    sleep(2)
    state = radio.transmit(message.c_str());
    if(state == RADIOLIB_ERR_NONE) {
        std::cout << "Message transmitted successfully!" << std::endl;
